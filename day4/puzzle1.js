var input = "[1518-05-28 00:59] wakes up\n" +
    "[1518-06-29 00:54] falls asleep\n" +
    "[1518-08-22 00:16] falls asleep\n" +
    "[1518-07-04 00:46] wakes up\n" +
    "[1518-09-02 00:00] Guard #2137 begins shift\n" +
    "[1518-05-01 00:45] falls asleep\n" +
    "[1518-08-15 00:47] wakes up\n" +
    "[1518-07-03 00:56] wakes up\n" +
    "[1518-11-17 00:02] Guard #563 begins shift\n" +
    "[1518-11-08 00:46] falls asleep\n" +
    "[1518-09-14 00:37] wakes up\n" +
    "[1518-03-06 00:49] wakes up\n" +
    "[1518-03-21 00:37] falls asleep\n" +
    "[1518-07-11 00:03] Guard #2083 begins shift\n" +
    "[1518-02-18 00:34] wakes up\n" +
    "[1518-06-02 00:38] wakes up\n" +
    "[1518-03-10 00:46] wakes up\n" +
    "[1518-10-12 00:23] falls asleep\n" +
    "[1518-03-02 00:55] falls asleep\n" +
    "[1518-10-06 00:17] wakes up\n" +
    "[1518-04-19 00:00] Guard #3511 begins shift\n" +
    "[1518-10-03 00:37] falls asleep\n" +
    "[1518-04-28 00:02] Guard #1499 begins shift\n" +
    "[1518-03-09 00:30] wakes up\n" +
    "[1518-08-17 00:00] Guard #2377 begins shift\n" +
    "[1518-07-04 00:58] wakes up\n" +
    "[1518-05-13 00:39] wakes up\n" +
    "[1518-07-06 00:41] falls asleep\n" +
    "[1518-08-06 23:57] Guard #983 begins shift\n" +
    "[1518-06-07 00:52] wakes up\n" +
    "[1518-10-04 00:00] Guard #1499 begins shift\n" +
    "[1518-08-17 00:21] falls asleep\n" +
    "[1518-02-18 00:17] falls asleep\n" +
    "[1518-03-23 00:57] wakes up\n" +
    "[1518-07-11 23:59] Guard #1709 begins shift\n" +
    "[1518-05-20 00:53] wakes up\n" +
    "[1518-11-22 00:50] falls asleep\n" +
    "[1518-10-28 00:16] wakes up\n" +
    "[1518-05-21 00:45] falls asleep\n" +
    "[1518-05-05 00:15] falls asleep\n" +
    "[1518-08-03 23:58] Guard #1499 begins shift\n" +
    "[1518-05-24 00:22] falls asleep\n" +
    "[1518-07-12 23:49] Guard #1499 begins shift\n" +
    "[1518-08-14 00:24] falls asleep\n" +
    "[1518-05-09 00:17] falls asleep\n" +
    "[1518-08-10 00:01] Guard #2591 begins shift\n" +
    "[1518-10-16 00:04] falls asleep\n" +
    "[1518-09-08 00:33] falls asleep\n" +
    "[1518-06-14 00:55] falls asleep\n" +
    "[1518-06-06 00:40] falls asleep\n" +
    "[1518-10-28 00:39] falls asleep\n" +
    "[1518-08-12 23:56] Guard #2267 begins shift\n" +
    "[1518-08-04 23:56] Guard #983 begins shift\n" +
    "[1518-09-04 00:34] falls asleep\n" +
    "[1518-06-14 00:46] falls asleep\n" +
    "[1518-09-29 00:22] wakes up\n" +
    "[1518-10-21 00:59] wakes up\n" +
    "[1518-07-27 00:59] wakes up\n" +
    "[1518-09-03 00:02] Guard #1499 begins shift\n" +
    "[1518-11-06 00:47] wakes up\n" +
    "[1518-07-12 00:52] falls asleep\n" +
    "[1518-03-06 00:06] falls asleep\n" +
    "[1518-10-17 00:46] wakes up\n" +
    "[1518-06-24 00:01] Guard #2663 begins shift\n" +
    "[1518-10-07 00:22] wakes up\n" +
    "[1518-11-06 00:02] Guard #3541 begins shift\n" +
    "[1518-05-21 00:49] wakes up\n" +
    "[1518-07-15 23:52] Guard #2591 begins shift\n" +
    "[1518-08-20 00:09] falls asleep\n" +
    "[1518-07-28 00:57] falls asleep\n" +
    "[1518-11-15 00:04] Guard #2083 begins shift\n" +
    "[1518-09-02 00:45] wakes up\n" +
    "[1518-07-08 00:30] falls asleep\n" +
    "[1518-07-24 00:03] Guard #2251 begins shift\n" +
    "[1518-05-22 00:01] falls asleep\n" +
    "[1518-05-28 00:47] falls asleep\n" +
    "[1518-08-08 00:40] falls asleep\n" +
    "[1518-03-06 00:14] falls asleep\n" +
    "[1518-07-03 00:02] Guard #2689 begins shift\n" +
    "[1518-02-17 00:48] wakes up\n" +
    "[1518-04-09 00:54] falls asleep\n" +
    "[1518-04-24 00:57] falls asleep\n" +
    "[1518-09-25 00:19] falls asleep\n" +
    "[1518-05-27 00:49] falls asleep\n" +
    "[1518-08-06 00:54] falls asleep\n" +
    "[1518-07-30 00:51] wakes up\n" +
    "[1518-08-26 00:17] falls asleep\n" +
    "[1518-10-05 00:44] wakes up\n" +
    "[1518-08-22 00:04] Guard #2251 begins shift\n" +
    "[1518-09-25 00:51] falls asleep\n" +
    "[1518-09-21 00:44] wakes up\n" +
    "[1518-04-02 00:59] wakes up\n" +
    "[1518-06-09 00:42] wakes up\n" +
    "[1518-03-23 00:34] wakes up\n" +
    "[1518-06-05 00:21] falls asleep\n" +
    "[1518-11-14 00:53] falls asleep\n" +
    "[1518-10-11 00:22] falls asleep\n" +
    "[1518-03-05 00:34] wakes up\n" +
    "[1518-05-24 00:02] Guard #2083 begins shift\n" +
    "[1518-11-03 23:58] Guard #2267 begins shift\n" +
    "[1518-05-09 00:21] wakes up\n" +
    "[1518-09-02 00:56] falls asleep\n" +
    "[1518-09-28 00:42] falls asleep\n" +
    "[1518-07-14 00:55] wakes up\n" +
    "[1518-07-02 00:15] falls asleep\n" +
    "[1518-10-13 00:34] falls asleep\n" +
    "[1518-02-22 00:03] falls asleep\n" +
    "[1518-04-04 23:57] Guard #2137 begins shift\n" +
    "[1518-06-05 00:46] falls asleep\n" +
    "[1518-02-14 00:53] wakes up\n" +
    "[1518-03-29 00:47] wakes up\n" +
    "[1518-10-19 00:42] wakes up\n" +
    "[1518-11-03 00:51] wakes up\n" +
    "[1518-02-15 23:47] Guard #79 begins shift\n" +
    "[1518-02-14 00:03] falls asleep\n" +
    "[1518-07-18 00:41] falls asleep\n" +
    "[1518-06-14 00:00] Guard #3541 begins shift\n" +
    "[1518-06-11 00:52] falls asleep\n" +
    "[1518-07-28 00:45] falls asleep\n" +
    "[1518-04-18 00:11] falls asleep\n" +
    "[1518-09-17 00:51] wakes up\n" +
    "[1518-04-03 00:56] wakes up\n" +
    "[1518-04-24 00:53] wakes up\n" +
    "[1518-04-30 00:41] falls asleep\n" +
    "[1518-05-11 00:00] Guard #631 begins shift\n" +
    "[1518-04-30 00:46] wakes up\n" +
    "[1518-04-30 00:20] wakes up\n" +
    "[1518-05-14 00:47] wakes up\n" +
    "[1518-11-11 23:56] Guard #2153 begins shift\n" +
    "[1518-10-29 00:57] falls asleep\n" +
    "[1518-03-11 00:02] Guard #2267 begins shift\n" +
    "[1518-10-07 00:29] falls asleep\n" +
    "[1518-03-26 00:48] wakes up\n" +
    "[1518-03-11 00:28] falls asleep\n" +
    "[1518-06-27 23:58] Guard #2713 begins shift\n" +
    "[1518-04-06 00:03] Guard #1499 begins shift\n" +
    "[1518-11-12 00:36] falls asleep\n" +
    "[1518-05-07 00:35] wakes up\n" +
    "[1518-06-18 00:43] wakes up\n" +
    "[1518-11-13 00:00] Guard #2663 begins shift\n" +
    "[1518-06-04 00:42] wakes up\n" +
    "[1518-11-16 00:13] falls asleep\n" +
    "[1518-06-22 00:54] wakes up\n" +
    "[1518-06-27 00:51] wakes up\n" +
    "[1518-08-28 00:38] wakes up\n" +
    "[1518-05-08 00:55] falls asleep\n" +
    "[1518-04-08 00:12] falls asleep\n" +
    "[1518-06-13 00:17] falls asleep\n" +
    "[1518-10-24 00:02] Guard #1259 begins shift\n" +
    "[1518-08-01 00:54] wakes up\n" +
    "[1518-11-15 00:07] falls asleep\n" +
    "[1518-05-15 00:29] falls asleep\n" +
    "[1518-07-13 00:51] wakes up\n" +
    "[1518-10-16 00:23] wakes up\n" +
    "[1518-03-21 00:58] wakes up\n" +
    "[1518-04-11 00:58] wakes up\n" +
    "[1518-02-17 00:39] falls asleep\n" +
    "[1518-08-18 00:21] falls asleep\n" +
    "[1518-04-01 00:11] falls asleep\n" +
    "[1518-09-06 23:58] Guard #1109 begins shift\n" +
    "[1518-06-03 00:38] falls asleep\n" +
    "[1518-07-30 00:46] falls asleep\n" +
    "[1518-04-21 00:42] falls asleep\n" +
    "[1518-04-29 00:16] falls asleep\n" +
    "[1518-09-22 00:43] falls asleep\n" +
    "[1518-07-04 00:12] falls asleep\n" +
    "[1518-09-06 00:40] wakes up\n" +
    "[1518-03-24 00:56] wakes up\n" +
    "[1518-08-20 00:23] falls asleep\n" +
    "[1518-05-22 00:13] falls asleep\n" +
    "[1518-03-15 00:53] wakes up\n" +
    "[1518-03-04 00:58] wakes up\n" +
    "[1518-08-15 00:46] falls asleep\n" +
    "[1518-04-09 00:10] falls asleep\n" +
    "[1518-07-14 00:09] falls asleep\n" +
    "[1518-04-27 00:19] falls asleep\n" +
    "[1518-10-17 00:04] Guard #2689 begins shift\n" +
    "[1518-05-27 00:57] wakes up\n" +
    "[1518-05-19 00:04] falls asleep\n" +
    "[1518-09-28 23:49] Guard #2591 begins shift\n" +
    "[1518-04-11 00:49] falls asleep\n" +
    "[1518-09-08 00:42] falls asleep\n" +
    "[1518-04-24 00:59] wakes up\n" +
    "[1518-07-01 00:55] falls asleep\n" +
    "[1518-02-19 00:55] wakes up\n" +
    "[1518-07-11 00:36] falls asleep\n" +
    "[1518-03-22 00:33] falls asleep\n" +
    "[1518-04-25 00:00] falls asleep\n" +
    "[1518-10-12 00:39] wakes up\n" +
    "[1518-07-26 00:23] falls asleep\n" +
    "[1518-05-26 00:53] wakes up\n" +
    "[1518-04-01 23:51] Guard #3511 begins shift\n" +
    "[1518-06-13 00:06] falls asleep\n" +
    "[1518-02-16 00:08] wakes up\n" +
    "[1518-09-07 23:51] Guard #2137 begins shift\n" +
    "[1518-09-15 00:04] Guard #1499 begins shift\n" +
    "[1518-11-10 23:56] Guard #2137 begins shift\n" +
    "[1518-05-08 00:58] wakes up\n" +
    "[1518-04-02 00:55] falls asleep\n" +
    "[1518-09-12 00:59] wakes up\n" +
    "[1518-04-13 00:28] wakes up\n" +
    "[1518-02-23 00:18] falls asleep\n" +
    "[1518-05-06 00:24] falls asleep\n" +
    "[1518-06-01 00:03] Guard #2083 begins shift\n" +
    "[1518-04-15 00:22] falls asleep\n" +
    "[1518-05-31 00:36] falls asleep\n" +
    "[1518-09-29 00:02] falls asleep\n" +
    "[1518-09-10 00:00] Guard #277 begins shift\n" +
    "[1518-05-31 00:56] wakes up\n" +
    "[1518-06-03 23:58] Guard #2083 begins shift\n" +
    "[1518-10-13 00:59] wakes up\n" +
    "[1518-03-15 00:42] falls asleep\n" +
    "[1518-09-22 00:01] Guard #2137 begins shift\n" +
    "[1518-10-08 00:25] wakes up\n" +
    "[1518-07-06 00:03] Guard #2797 begins shift\n" +
    "[1518-03-24 00:39] falls asleep\n" +
    "[1518-09-01 00:02] Guard #3079 begins shift\n" +
    "[1518-11-04 00:51] falls asleep\n" +
    "[1518-09-28 00:10] falls asleep\n" +
    "[1518-02-27 00:44] wakes up\n" +
    "[1518-11-09 00:27] wakes up\n" +
    "[1518-06-24 00:42] falls asleep\n" +
    "[1518-04-08 00:57] wakes up\n" +
    "[1518-10-11 00:03] Guard #1259 begins shift\n" +
    "[1518-07-26 23:58] Guard #2797 begins shift\n" +
    "[1518-11-23 00:53] wakes up\n" +
    "[1518-07-22 00:40] falls asleep\n" +
    "[1518-05-11 00:52] wakes up\n" +
    "[1518-03-31 00:23] wakes up\n" +
    "[1518-03-02 00:39] wakes up\n" +
    "[1518-03-31 23:56] Guard #1499 begins shift\n" +
    "[1518-09-05 00:31] falls asleep\n" +
    "[1518-11-07 00:04] Guard #2797 begins shift\n" +
    "[1518-06-15 00:29] wakes up\n" +
    "[1518-07-07 00:03] falls asleep\n" +
    "[1518-10-02 00:52] wakes up\n" +
    "[1518-03-20 00:26] falls asleep\n" +
    "[1518-02-27 00:53] wakes up\n" +
    "[1518-11-02 00:04] Guard #1109 begins shift\n" +
    "[1518-05-20 00:11] falls asleep\n" +
    "[1518-10-18 00:04] Guard #2251 begins shift\n" +
    "[1518-05-16 00:00] Guard #563 begins shift\n" +
    "[1518-10-02 00:35] falls asleep\n" +
    "[1518-04-13 00:44] wakes up\n" +
    "[1518-09-12 00:00] Guard #3541 begins shift\n" +
    "[1518-09-26 00:57] wakes up\n" +
    "[1518-06-01 00:19] falls asleep\n" +
    "[1518-10-08 00:53] wakes up\n" +
    "[1518-02-27 00:36] falls asleep\n" +
    "[1518-03-22 00:51] wakes up\n" +
    "[1518-08-30 00:00] falls asleep\n" +
    "[1518-08-25 00:24] wakes up\n" +
    "[1518-10-02 00:00] Guard #277 begins shift\n" +
    "[1518-10-29 00:59] wakes up\n" +
    "[1518-02-14 00:41] falls asleep\n" +
    "[1518-05-07 00:00] falls asleep\n" +
    "[1518-06-25 00:54] falls asleep\n" +
    "[1518-08-20 00:01] Guard #631 begins shift\n" +
    "[1518-02-24 23:52] Guard #79 begins shift\n" +
    "[1518-05-31 00:03] Guard #1259 begins shift\n" +
    "[1518-08-22 23:50] Guard #631 begins shift\n" +
    "[1518-08-18 00:40] wakes up\n" +
    "[1518-04-23 00:31] falls asleep\n" +
    "[1518-02-16 00:04] falls asleep\n" +
    "[1518-03-30 00:14] falls asleep\n" +
    "[1518-11-16 00:44] falls asleep\n" +
    "[1518-04-04 00:04] falls asleep\n" +
    "[1518-07-05 00:10] falls asleep\n" +
    "[1518-08-17 00:47] wakes up\n" +
    "[1518-05-20 00:38] wakes up\n" +
    "[1518-10-25 00:29] falls asleep\n" +
    "[1518-07-06 00:37] wakes up\n" +
    "[1518-04-24 00:04] Guard #3511 begins shift\n" +
    "[1518-02-26 23:56] Guard #277 begins shift\n" +
    "[1518-11-19 00:58] wakes up\n" +
    "[1518-05-08 00:29] falls asleep\n" +
    "[1518-08-19 00:44] falls asleep\n" +
    "[1518-02-28 00:27] falls asleep\n" +
    "[1518-05-23 00:36] falls asleep\n" +
    "[1518-11-08 00:54] wakes up\n" +
    "[1518-07-15 00:10] falls asleep\n" +
    "[1518-09-03 23:56] Guard #2153 begins shift\n" +
    "[1518-05-20 00:41] falls asleep\n" +
    "[1518-05-10 00:05] falls asleep\n" +
    "[1518-04-04 00:41] falls asleep\n" +
    "[1518-10-09 00:45] wakes up\n" +
    "[1518-06-14 00:43] wakes up\n" +
    "[1518-09-14 00:20] falls asleep\n" +
    "[1518-07-03 00:39] wakes up\n" +
    "[1518-05-07 00:30] falls asleep\n" +
    "[1518-03-25 00:17] falls asleep\n" +
    "[1518-03-31 00:16] falls asleep\n" +
    "[1518-06-21 00:14] falls asleep\n" +
    "[1518-10-08 00:47] falls asleep\n" +
    "[1518-10-04 00:43] falls asleep\n" +
    "[1518-10-06 00:02] Guard #2153 begins shift\n" +
    "[1518-04-01 00:47] wakes up\n" +
    "[1518-05-14 00:31] wakes up\n" +
    "[1518-03-28 00:41] wakes up\n" +
    "[1518-10-16 00:57] wakes up\n" +
    "[1518-03-12 00:04] Guard #2797 begins shift\n" +
    "[1518-08-24 00:39] wakes up\n" +
    "[1518-05-17 00:01] Guard #1499 begins shift\n" +
    "[1518-03-16 00:53] wakes up\n" +
    "[1518-07-07 00:56] wakes up\n" +
    "[1518-08-25 00:00] Guard #1709 begins shift\n" +
    "[1518-06-15 00:57] wakes up\n" +
    "[1518-03-07 00:21] falls asleep\n" +
    "[1518-07-02 00:16] wakes up\n" +
    "[1518-09-17 00:17] falls asleep\n" +
    "[1518-07-29 00:50] wakes up\n" +
    "[1518-09-22 23:48] Guard #2137 begins shift\n" +
    "[1518-03-06 23:46] Guard #2377 begins shift\n" +
    "[1518-10-26 00:20] falls asleep\n" +
    "[1518-07-12 00:57] wakes up\n" +
    "[1518-09-29 00:59] wakes up\n" +
    "[1518-09-23 00:00] falls asleep\n" +
    "[1518-10-20 00:54] wakes up\n" +
    "[1518-11-11 00:44] falls asleep\n" +
    "[1518-05-08 00:51] wakes up\n" +
    "[1518-03-30 00:42] falls asleep\n" +
    "[1518-06-22 00:00] Guard #3541 begins shift\n" +
    "[1518-02-19 00:43] wakes up\n" +
    "[1518-09-04 00:26] wakes up\n" +
    "[1518-02-20 00:02] Guard #79 begins shift\n" +
    "[1518-07-24 00:15] falls asleep\n" +
    "[1518-05-17 00:53] wakes up\n" +
    "[1518-10-19 00:39] falls asleep\n" +
    "[1518-10-14 00:50] falls asleep\n" +
    "[1518-03-30 00:31] wakes up\n" +
    "[1518-06-03 00:54] falls asleep\n" +
    "[1518-02-24 00:28] falls asleep\n" +
    "[1518-10-05 00:38] falls asleep\n" +
    "[1518-06-28 23:59] Guard #2591 begins shift\n" +
    "[1518-07-11 00:58] wakes up\n" +
    "[1518-07-22 00:52] falls asleep\n" +
    "[1518-11-01 00:53] wakes up\n" +
    "[1518-10-15 00:35] falls asleep\n" +
    "[1518-03-15 00:02] falls asleep\n" +
    "[1518-09-09 00:00] Guard #563 begins shift\n" +
    "[1518-09-04 00:06] falls asleep\n" +
    "[1518-03-01 00:03] falls asleep\n" +
    "[1518-02-22 23:59] Guard #2797 begins shift\n" +
    "[1518-10-09 00:39] wakes up\n" +
    "[1518-03-21 23:57] Guard #1259 begins shift\n" +
    "[1518-04-04 00:37] wakes up\n" +
    "[1518-05-21 23:50] Guard #2267 begins shift\n" +
    "[1518-09-28 00:36] wakes up\n" +
    "[1518-05-28 00:39] falls asleep\n" +
    "[1518-02-24 00:02] Guard #79 begins shift\n" +
    "[1518-10-11 00:56] wakes up\n" +
    "[1518-06-19 23:58] Guard #2689 begins shift\n" +
    "[1518-09-08 00:26] wakes up\n" +
    "[1518-11-14 00:30] wakes up\n" +
    "[1518-04-12 00:57] wakes up\n" +
    "[1518-06-06 00:46] wakes up\n" +
    "[1518-04-18 00:51] wakes up\n" +
    "[1518-05-21 00:33] wakes up\n" +
    "[1518-07-03 00:47] falls asleep\n" +
    "[1518-07-06 00:23] falls asleep\n" +
    "[1518-04-22 00:03] falls asleep\n" +
    "[1518-05-02 00:35] wakes up\n" +
    "[1518-10-04 00:54] falls asleep\n" +
    "[1518-09-08 00:38] wakes up\n" +
    "[1518-05-03 00:59] wakes up\n" +
    "[1518-11-09 00:25] falls asleep\n" +
    "[1518-10-26 00:49] wakes up\n" +
    "[1518-11-13 00:31] falls asleep\n" +
    "[1518-03-23 23:59] Guard #277 begins shift\n" +
    "[1518-07-27 00:43] falls asleep\n" +
    "[1518-10-28 00:03] falls asleep\n" +
    "[1518-07-25 00:10] falls asleep\n" +
    "[1518-08-17 23:58] Guard #2663 begins shift\n" +
    "[1518-09-24 00:50] wakes up\n" +
    "[1518-06-13 00:45] falls asleep\n" +
    "[1518-08-15 00:00] Guard #1709 begins shift\n" +
    "[1518-05-14 00:30] falls asleep\n" +
    "[1518-09-28 00:00] Guard #563 begins shift\n" +
    "[1518-07-22 00:42] wakes up\n" +
    "[1518-08-29 23:53] Guard #2689 begins shift\n" +
    "[1518-07-20 23:46] Guard #2377 begins shift\n" +
    "[1518-06-14 00:59] wakes up\n" +
    "[1518-09-21 00:35] falls asleep\n" +
    "[1518-03-09 00:48] wakes up\n" +
    "[1518-10-04 00:48] wakes up\n" +
    "[1518-04-17 00:45] wakes up\n" +
    "[1518-03-09 23:54] Guard #2797 begins shift\n" +
    "[1518-08-12 00:01] Guard #2713 begins shift\n" +
    "[1518-06-20 23:59] Guard #1259 begins shift\n" +
    "[1518-08-31 00:57] falls asleep\n" +
    "[1518-06-19 00:48] wakes up\n" +
    "[1518-06-14 23:48] Guard #2663 begins shift\n" +
    "[1518-10-22 00:46] wakes up\n" +
    "[1518-07-16 00:13] wakes up\n" +
    "[1518-09-06 00:10] falls asleep\n" +
    "[1518-03-19 00:57] wakes up\n" +
    "[1518-09-30 23:59] Guard #277 begins shift\n" +
    "[1518-04-07 00:51] falls asleep\n" +
    "[1518-04-03 00:04] Guard #277 begins shift\n" +
    "[1518-04-20 00:53] wakes up\n" +
    "[1518-06-24 00:59] wakes up\n" +
    "[1518-10-10 00:43] wakes up\n" +
    "[1518-06-30 00:59] wakes up\n" +
    "[1518-09-15 23:56] Guard #2797 begins shift\n" +
    "[1518-10-21 00:04] Guard #983 begins shift\n" +
    "[1518-06-25 23:52] Guard #2591 begins shift\n" +
    "[1518-04-21 00:53] wakes up\n" +
    "[1518-02-28 00:55] wakes up\n" +
    "[1518-03-11 00:09] falls asleep\n" +
    "[1518-04-12 00:34] falls asleep\n" +
    "[1518-09-20 00:28] wakes up\n" +
    "[1518-09-15 00:49] wakes up\n" +
    "[1518-05-30 00:58] wakes up\n" +
    "[1518-05-02 00:11] wakes up\n" +
    "[1518-05-20 00:00] Guard #631 begins shift\n" +
    "[1518-06-08 00:42] falls asleep\n" +
    "[1518-10-26 00:00] Guard #2083 begins shift\n" +
    "[1518-06-25 00:57] wakes up\n" +
    "[1518-06-29 00:58] wakes up\n" +
    "[1518-05-11 00:39] wakes up\n" +
    "[1518-09-20 00:23] falls asleep\n" +
    "[1518-08-10 00:56] wakes up\n" +
    "[1518-08-16 00:14] falls asleep\n" +
    "[1518-11-22 00:46] wakes up\n" +
    "[1518-11-21 23:46] Guard #1259 begins shift\n" +
    "[1518-05-16 00:29] falls asleep\n" +
    "[1518-08-05 00:55] falls asleep\n" +
    "[1518-04-29 00:53] wakes up\n" +
    "[1518-06-07 00:01] Guard #2377 begins shift\n" +
    "[1518-08-15 00:55] falls asleep\n" +
    "[1518-06-21 00:33] wakes up\n" +
    "[1518-07-29 00:01] falls asleep\n" +
    "[1518-09-26 00:39] falls asleep\n" +
    "[1518-03-14 00:57] wakes up\n" +
    "[1518-04-25 00:37] wakes up\n" +
    "[1518-04-06 00:32] falls asleep\n" +
    "[1518-11-14 00:37] falls asleep\n" +
    "[1518-08-31 00:58] wakes up\n" +
    "[1518-11-10 00:27] falls asleep\n" +
    "[1518-05-08 00:00] Guard #563 begins shift\n" +
    "[1518-02-28 00:38] wakes up\n" +
    "[1518-05-05 00:49] falls asleep\n" +
    "[1518-02-24 00:41] wakes up\n" +
    "[1518-11-21 00:08] falls asleep\n" +
    "[1518-08-29 00:46] falls asleep\n" +
    "[1518-09-24 00:54] falls asleep\n" +
    "[1518-03-11 00:29] wakes up\n" +
    "[1518-08-04 00:36] wakes up\n" +
    "[1518-07-22 00:02] Guard #2137 begins shift\n" +
    "[1518-07-28 00:59] wakes up\n" +
    "[1518-02-28 00:42] falls asleep\n" +
    "[1518-07-02 00:50] falls asleep\n" +
    "[1518-05-18 00:53] wakes up\n" +
    "[1518-07-15 00:23] wakes up\n" +
    "[1518-08-01 00:10] falls asleep\n" +
    "[1518-04-28 00:39] wakes up\n" +
    "[1518-09-04 00:13] falls asleep\n" +
    "[1518-03-04 00:50] falls asleep\n" +
    "[1518-11-07 00:47] wakes up\n" +
    "[1518-05-26 23:49] Guard #3541 begins shift\n" +
    "[1518-05-28 00:00] Guard #2251 begins shift\n" +
    "[1518-07-17 23:50] Guard #2377 begins shift\n" +
    "[1518-03-25 00:50] wakes up\n" +
    "[1518-09-17 00:00] falls asleep\n" +
    "[1518-05-21 00:04] Guard #563 begins shift\n" +
    "[1518-03-19 00:44] wakes up\n" +
    "[1518-11-20 00:39] wakes up\n" +
    "[1518-03-07 00:49] wakes up\n" +
    "[1518-03-14 23:53] Guard #2797 begins shift\n" +
    "[1518-10-02 23:59] Guard #983 begins shift\n" +
    "[1518-08-06 00:36] wakes up\n" +
    "[1518-07-31 00:26] falls asleep\n" +
    "[1518-07-28 00:53] wakes up\n" +
    "[1518-07-11 00:30] wakes up\n" +
    "[1518-05-02 00:19] falls asleep\n" +
    "[1518-06-21 00:56] wakes up\n" +
    "[1518-04-10 00:24] falls asleep\n" +
    "[1518-10-18 00:15] falls asleep\n" +
    "[1518-04-17 00:00] Guard #2137 begins shift\n" +
    "[1518-07-28 23:49] Guard #2797 begins shift\n" +
    "[1518-11-19 00:00] Guard #2153 begins shift\n" +
    "[1518-06-04 00:13] falls asleep\n" +
    "[1518-06-13 00:19] wakes up\n" +
    "[1518-09-15 00:32] falls asleep\n" +
    "[1518-07-11 00:49] falls asleep\n" +
    "[1518-03-30 00:52] wakes up\n" +
    "[1518-07-01 23:59] Guard #1259 begins shift\n" +
    "[1518-07-13 00:00] falls asleep\n" +
    "[1518-02-27 00:32] wakes up\n" +
    "[1518-10-30 00:15] wakes up\n" +
    "[1518-02-27 00:48] falls asleep\n" +
    "[1518-09-04 00:55] wakes up\n" +
    "[1518-09-19 00:04] falls asleep\n" +
    "[1518-07-15 00:53] wakes up\n" +
    "[1518-09-04 00:50] falls asleep\n" +
    "[1518-11-13 00:55] wakes up\n" +
    "[1518-05-13 00:59] wakes up\n" +
    "[1518-10-31 00:49] wakes up\n" +
    "[1518-10-24 00:54] falls asleep\n" +
    "[1518-04-02 00:51] wakes up\n" +
    "[1518-10-26 00:16] wakes up\n" +
    "[1518-10-07 23:53] Guard #2797 begins shift\n" +
    "[1518-05-30 00:30] wakes up\n" +
    "[1518-02-16 23:51] Guard #1709 begins shift\n" +
    "[1518-11-23 00:40] falls asleep\n" +
    "[1518-04-27 00:00] Guard #563 begins shift\n" +
    "[1518-04-28 00:30] falls asleep\n" +
    "[1518-02-21 00:38] wakes up\n" +
    "[1518-05-13 00:07] falls asleep\n" +
    "[1518-07-11 00:12] falls asleep\n" +
    "[1518-04-02 00:03] falls asleep\n" +
    "[1518-03-16 00:08] falls asleep\n" +
    "[1518-10-15 00:43] wakes up\n" +
    "[1518-02-15 00:47] wakes up\n" +
    "[1518-11-01 00:23] falls asleep\n" +
    "[1518-07-05 00:37] falls asleep\n" +
    "[1518-10-05 00:04] Guard #2137 begins shift\n" +
    "[1518-10-24 00:17] falls asleep\n" +
    "[1518-07-01 00:15] falls asleep\n" +
    "[1518-02-25 00:11] wakes up\n" +
    "[1518-06-09 00:34] falls asleep\n" +
    "[1518-06-24 00:36] wakes up\n" +
    "[1518-02-26 00:00] Guard #2689 begins shift\n" +
    "[1518-10-06 00:47] wakes up\n" +
    "[1518-10-14 00:02] Guard #2689 begins shift\n" +
    "[1518-04-29 00:00] Guard #2251 begins shift\n" +
    "[1518-03-10 00:57] wakes up\n" +
    "[1518-10-20 00:03] Guard #1259 begins shift\n" +
    "[1518-08-13 00:53] wakes up\n" +
    "[1518-03-18 00:34] falls asleep\n" +
    "[1518-08-21 00:58] wakes up\n" +
    "[1518-09-12 00:18] wakes up\n" +
    "[1518-05-12 00:02] Guard #2153 begins shift\n" +
    "[1518-07-15 00:04] Guard #2153 begins shift\n" +
    "[1518-06-03 00:02] Guard #277 begins shift\n" +
    "[1518-08-02 00:04] Guard #2797 begins shift\n" +
    "[1518-06-17 23:57] Guard #3541 begins shift\n" +
    "[1518-11-16 00:03] Guard #277 begins shift\n" +
    "[1518-07-18 00:26] wakes up\n" +
    "[1518-03-09 00:21] falls asleep\n" +
    "[1518-11-03 00:00] Guard #3511 begins shift\n" +
    "[1518-03-02 00:34] falls asleep\n" +
    "[1518-11-11 00:55] wakes up\n" +
    "[1518-05-23 00:04] Guard #2251 begins shift\n" +
    "[1518-06-12 00:42] falls asleep\n" +
    "[1518-09-11 00:55] wakes up\n" +
    "[1518-02-19 00:07] falls asleep\n" +
    "[1518-10-21 00:41] falls asleep\n" +
    "[1518-10-07 00:09] falls asleep\n" +
    "[1518-04-22 00:57] wakes up\n" +
    "[1518-02-16 00:13] falls asleep\n" +
    "[1518-05-16 00:48] falls asleep\n" +
    "[1518-08-21 00:53] falls asleep\n" +
    "[1518-04-06 00:50] falls asleep\n" +
    "[1518-10-09 00:42] falls asleep\n" +
    "[1518-07-01 00:26] wakes up\n" +
    "[1518-09-21 00:01] Guard #2797 begins shift\n" +
    "[1518-04-19 00:56] wakes up\n" +
    "[1518-06-18 00:31] falls asleep\n" +
    "[1518-04-11 00:52] wakes up\n" +
    "[1518-09-24 00:37] wakes up\n" +
    "[1518-07-05 00:29] wakes up\n" +
    "[1518-02-25 00:00] falls asleep\n" +
    "[1518-06-17 00:31] falls asleep\n" +
    "[1518-10-15 00:04] Guard #3511 begins shift\n" +
    "[1518-05-05 00:53] wakes up\n" +
    "[1518-03-08 00:31] wakes up\n" +
    "[1518-03-18 00:59] wakes up\n" +
    "[1518-07-15 00:27] falls asleep\n" +
    "[1518-11-17 00:59] wakes up\n" +
    "[1518-09-11 00:26] falls asleep\n" +
    "[1518-08-25 00:34] falls asleep\n" +
    "[1518-04-09 00:57] wakes up\n" +
    "[1518-04-23 00:02] Guard #2083 begins shift\n" +
    "[1518-03-18 23:58] Guard #2153 begins shift\n" +
    "[1518-11-15 00:48] wakes up\n" +
    "[1518-08-11 00:54] wakes up\n" +
    "[1518-03-28 00:59] wakes up\n" +
    "[1518-04-30 00:10] falls asleep\n" +
    "[1518-09-26 00:01] Guard #3541 begins shift\n" +
    "[1518-03-04 23:52] Guard #2377 begins shift\n" +
    "[1518-03-30 00:00] Guard #2689 begins shift\n" +
    "[1518-06-06 00:18] falls asleep\n" +
    "[1518-05-07 00:57] wakes up\n" +
    "[1518-07-19 00:48] wakes up\n" +
    "[1518-05-09 23:51] Guard #2377 begins shift\n" +
    "[1518-05-01 00:57] wakes up\n" +
    "[1518-03-02 00:01] Guard #1499 begins shift\n" +
    "[1518-03-23 00:22] falls asleep\n" +
    "[1518-06-24 00:27] falls asleep\n" +
    "[1518-09-27 00:49] wakes up\n" +
    "[1518-02-17 00:32] wakes up\n" +
    "[1518-07-21 00:47] wakes up\n" +
    "[1518-10-22 00:37] falls asleep\n" +
    "[1518-04-05 00:14] wakes up\n" +
    "[1518-10-15 23:49] Guard #79 begins shift\n" +
    "[1518-08-04 00:54] wakes up\n" +
    "[1518-03-07 00:31] falls asleep\n" +
    "[1518-04-30 23:56] Guard #2689 begins shift\n" +
    "[1518-06-15 00:05] falls asleep\n" +
    "[1518-10-16 00:53] falls asleep\n" +
    "[1518-07-18 00:03] falls asleep\n" +
    "[1518-03-07 00:02] falls asleep\n" +
    "[1518-07-16 23:58] Guard #1109 begins shift\n" +
    "[1518-05-29 00:52] falls asleep\n" +
    "[1518-03-27 23:58] Guard #3511 begins shift\n" +
    "[1518-03-27 00:16] falls asleep\n" +
    "[1518-07-21 00:03] falls asleep\n" +
    "[1518-02-14 00:22] wakes up\n" +
    "[1518-08-26 00:35] wakes up\n" +
    "[1518-09-19 00:50] wakes up\n" +
    "[1518-09-10 00:24] falls asleep\n" +
    "[1518-02-20 23:59] Guard #3511 begins shift\n" +
    "[1518-04-19 00:36] falls asleep\n" +
    "[1518-09-18 00:38] falls asleep\n" +
    "[1518-04-15 00:59] wakes up\n" +
    "[1518-03-07 00:06] wakes up\n" +
    "[1518-07-14 00:43] falls asleep\n" +
    "[1518-05-29 00:55] wakes up\n" +
    "[1518-07-25 23:58] Guard #1709 begins shift\n" +
    "[1518-05-12 23:56] Guard #277 begins shift\n" +
    "[1518-11-04 00:55] wakes up\n" +
    "[1518-07-27 00:52] wakes up\n" +
    "[1518-06-08 00:00] Guard #2689 begins shift\n" +
    "[1518-04-14 00:56] wakes up\n" +
    "[1518-10-07 00:03] Guard #2377 begins shift\n" +
    "[1518-10-25 00:51] wakes up\n" +
    "[1518-05-17 23:57] Guard #79 begins shift\n" +
    "[1518-04-07 00:35] wakes up\n" +
    "[1518-10-23 00:01] Guard #563 begins shift\n" +
    "[1518-06-13 00:59] wakes up\n" +
    "[1518-06-13 00:02] Guard #2591 begins shift\n" +
    "[1518-08-19 00:58] wakes up\n" +
    "[1518-03-12 00:30] falls asleep\n" +
    "[1518-06-12 00:12] wakes up\n" +
    "[1518-10-01 00:18] falls asleep\n" +
    "[1518-08-30 00:34] falls asleep\n" +
    "[1518-02-15 00:02] Guard #2267 begins shift\n" +
    "[1518-04-20 23:59] Guard #2267 begins shift\n" +
    "[1518-02-17 00:01] falls asleep\n" +
    "[1518-06-17 00:54] wakes up\n" +
    "[1518-11-16 00:41] wakes up\n" +
    "[1518-04-03 23:50] Guard #1499 begins shift\n" +
    "[1518-04-20 00:01] falls asleep\n" +
    "[1518-07-18 00:47] wakes up\n" +
    "[1518-08-16 00:03] Guard #2251 begins shift\n" +
    "[1518-04-28 00:38] falls asleep\n" +
    "[1518-10-21 00:50] wakes up\n" +
    "[1518-08-26 23:58] Guard #2251 begins shift\n" +
    "[1518-09-04 00:36] wakes up\n" +
    "[1518-04-03 00:36] falls asleep\n" +
    "[1518-03-11 00:23] wakes up\n" +
    "[1518-09-16 00:44] wakes up\n" +
    "[1518-02-26 00:52] wakes up\n" +
    "[1518-08-06 00:08] falls asleep\n" +
    "[1518-10-10 00:38] falls asleep\n" +
    "[1518-05-30 00:08] falls asleep\n" +
    "[1518-07-29 23:52] Guard #2377 begins shift\n" +
    "[1518-10-23 00:51] falls asleep\n" +
    "[1518-04-27 00:55] wakes up\n" +
    "[1518-05-19 00:54] wakes up\n" +
    "[1518-10-14 00:51] wakes up\n" +
    "[1518-06-29 00:48] wakes up\n" +
    "[1518-03-03 00:27] falls asleep\n" +
    "[1518-04-30 00:03] Guard #563 begins shift\n" +
    "[1518-09-02 00:58] wakes up\n" +
    "[1518-10-29 00:00] Guard #2663 begins shift\n" +
    "[1518-08-16 00:58] wakes up\n" +
    "[1518-05-18 00:11] falls asleep\n" +
    "[1518-05-01 00:38] wakes up\n" +
    "[1518-07-31 00:01] Guard #2663 begins shift\n" +
    "[1518-11-12 00:59] wakes up\n" +
    "[1518-11-18 00:03] Guard #2713 begins shift\n" +
    "[1518-03-25 23:58] Guard #2137 begins shift\n" +
    "[1518-06-10 00:40] falls asleep\n" +
    "[1518-06-14 00:38] falls asleep\n" +
    "[1518-05-13 00:55] falls asleep\n" +
    "[1518-08-20 00:15] wakes up\n" +
    "[1518-11-08 23:58] Guard #1709 begins shift\n" +
    "[1518-03-14 00:17] falls asleep\n" +
    "[1518-04-09 23:56] Guard #2083 begins shift\n" +
    "[1518-03-12 00:54] wakes up\n" +
    "[1518-11-08 00:42] wakes up\n" +
    "[1518-07-30 00:35] wakes up\n" +
    "[1518-05-14 00:02] Guard #2797 begins shift\n" +
    "[1518-05-05 00:03] Guard #2591 begins shift\n" +
    "[1518-05-29 00:37] wakes up\n" +
    "[1518-02-21 00:43] falls asleep\n" +
    "[1518-06-12 00:36] wakes up\n" +
    "[1518-07-26 00:46] wakes up\n" +
    "[1518-06-29 00:20] falls asleep\n" +
    "[1518-09-17 00:14] wakes up\n" +
    "[1518-06-05 23:59] Guard #79 begins shift\n" +
    "[1518-10-20 00:17] falls asleep\n" +
    "[1518-06-14 00:48] wakes up\n" +
    "[1518-09-04 00:07] wakes up\n" +
    "[1518-08-26 00:00] Guard #2689 begins shift\n" +
    "[1518-05-10 00:34] wakes up\n" +
    "[1518-04-05 00:56] wakes up\n" +
    "[1518-06-26 00:00] falls asleep\n" +
    "[1518-06-29 23:53] Guard #2267 begins shift\n" +
    "[1518-11-05 00:51] wakes up\n" +
    "[1518-04-19 00:28] wakes up\n" +
    "[1518-08-31 00:14] falls asleep\n" +
    "[1518-05-04 00:08] falls asleep\n" +
    "[1518-03-02 00:57] wakes up\n" +
    "[1518-10-30 00:07] falls asleep\n" +
    "[1518-08-03 00:01] Guard #1499 begins shift\n" +
    "[1518-11-20 00:00] Guard #2689 begins shift\n" +
    "[1518-07-06 00:53] wakes up\n" +
    "[1518-10-07 00:43] wakes up\n" +
    "[1518-07-10 00:13] falls asleep\n" +
    "[1518-07-13 00:34] falls asleep\n" +
    "[1518-03-05 23:57] Guard #2251 begins shift\n" +
    "[1518-06-03 00:46] wakes up\n" +
    "[1518-06-05 00:58] wakes up\n" +
    "[1518-08-13 00:49] falls asleep\n" +
    "[1518-09-27 00:02] Guard #277 begins shift\n" +
    "[1518-05-30 00:03] Guard #2377 begins shift\n" +
    "[1518-09-16 23:51] Guard #563 begins shift\n" +
    "[1518-05-15 00:00] Guard #1499 begins shift\n" +
    "[1518-08-27 00:55] wakes up\n" +
    "[1518-04-05 00:40] falls asleep\n" +
    "[1518-02-21 23:50] Guard #2663 begins shift\n" +
    "[1518-10-18 00:58] wakes up\n" +
    "[1518-09-05 23:56] Guard #2377 begins shift\n" +
    "[1518-07-20 00:02] Guard #2251 begins shift\n" +
    "[1518-08-02 00:16] falls asleep\n" +
    "[1518-10-27 00:35] wakes up\n" +
    "[1518-06-11 00:57] wakes up\n" +
    "[1518-10-16 00:36] falls asleep\n" +
    "[1518-03-18 00:30] wakes up\n" +
    "[1518-10-06 00:10] falls asleep\n" +
    "[1518-08-04 00:31] falls asleep\n" +
    "[1518-03-15 00:31] wakes up\n" +
    "[1518-09-09 00:42] wakes up\n" +
    "[1518-11-23 00:31] wakes up\n" +
    "[1518-08-02 00:29] wakes up\n" +
    "[1518-09-10 00:52] wakes up\n" +
    "[1518-07-04 00:50] falls asleep\n" +
    "[1518-07-31 00:53] wakes up\n" +
    "[1518-03-13 23:56] Guard #631 begins shift\n" +
    "[1518-07-20 00:12] falls asleep\n" +
    "[1518-03-27 00:45] wakes up\n" +
    "[1518-09-25 00:55] wakes up\n" +
    "[1518-07-05 00:47] wakes up\n" +
    "[1518-08-01 00:04] Guard #2153 begins shift\n" +
    "[1518-09-03 00:26] falls asleep\n" +
    "[1518-03-31 00:00] Guard #2267 begins shift\n" +
    "[1518-10-04 00:33] falls asleep\n" +
    "[1518-03-08 00:59] wakes up\n" +
    "[1518-09-12 00:10] falls asleep\n" +
    "[1518-10-25 00:09] falls asleep\n" +
    "[1518-05-01 00:25] falls asleep\n" +
    "[1518-10-09 00:53] wakes up\n" +
    "[1518-11-01 00:00] Guard #2153 begins shift\n" +
    "[1518-05-03 00:01] Guard #1499 begins shift\n" +
    "[1518-11-14 00:17] wakes up\n" +
    "[1518-09-24 00:56] wakes up\n" +
    "[1518-09-25 00:01] Guard #2663 begins shift\n" +
    "[1518-03-17 00:04] falls asleep\n" +
    "[1518-03-24 00:35] wakes up\n" +
    "[1518-04-26 00:04] Guard #1259 begins shift\n" +
    "[1518-04-19 00:18] falls asleep\n" +
    "[1518-08-31 00:50] wakes up\n" +
    "[1518-02-26 00:12] falls asleep\n" +
    "[1518-03-13 00:01] Guard #1109 begins shift\n" +
    "[1518-09-30 00:04] falls asleep\n" +
    "[1518-10-28 00:51] wakes up\n" +
    "[1518-05-07 00:12] falls asleep\n" +
    "[1518-08-29 00:53] wakes up\n" +
    "[1518-09-30 00:46] wakes up\n" +
    "[1518-05-22 00:52] wakes up\n" +
    "[1518-08-23 00:03] falls asleep\n" +
    "[1518-09-08 00:48] wakes up\n" +
    "[1518-05-18 23:48] Guard #2267 begins shift\n" +
    "[1518-10-21 00:56] falls asleep\n" +
    "[1518-10-04 00:59] wakes up\n" +
    "[1518-03-16 00:00] Guard #631 begins shift\n" +
    "[1518-07-28 00:02] Guard #2083 begins shift\n" +
    "[1518-07-22 23:58] Guard #3079 begins shift\n" +
    "[1518-05-26 00:43] falls asleep\n" +
    "[1518-06-09 00:47] wakes up\n" +
    "[1518-10-26 23:53] Guard #1499 begins shift\n" +
    "[1518-03-05 00:02] falls asleep\n" +
    "[1518-05-11 00:24] falls asleep\n" +
    "[1518-07-10 00:22] wakes up\n" +
    "[1518-11-22 00:00] falls asleep\n" +
    "[1518-07-03 00:14] falls asleep\n" +
    "[1518-08-30 00:59] wakes up\n" +
    "[1518-04-19 00:41] wakes up\n" +
    "[1518-08-05 00:56] wakes up\n" +
    "[1518-08-11 00:03] Guard #3511 begins shift\n" +
    "[1518-10-04 00:38] wakes up\n" +
    "[1518-11-22 00:24] falls asleep\n" +
    "[1518-03-09 00:03] wakes up\n" +
    "[1518-06-05 00:01] Guard #2797 begins shift\n" +
    "[1518-07-14 00:38] wakes up\n" +
    "[1518-03-05 00:29] falls asleep\n" +
    "[1518-03-25 00:00] Guard #983 begins shift\n" +
    "[1518-06-30 00:00] falls asleep\n" +
    "[1518-02-19 00:51] falls asleep\n" +
    "[1518-11-10 00:59] wakes up\n" +
    "[1518-06-10 00:49] wakes up\n" +
    "[1518-07-11 00:54] wakes up\n" +
    "[1518-04-17 00:24] falls asleep\n" +
    "[1518-10-10 00:04] Guard #2083 begins shift\n" +
    "[1518-10-29 00:19] falls asleep\n" +
    "[1518-07-19 00:02] falls asleep\n" +
    "[1518-03-28 00:54] falls asleep\n" +
    "[1518-06-20 00:10] falls asleep\n" +
    "[1518-03-17 00:46] wakes up\n" +
    "[1518-06-17 00:03] falls asleep\n" +
    "[1518-11-20 00:28] falls asleep\n" +
    "[1518-05-12 00:27] falls asleep\n" +
    "[1518-08-11 00:13] falls asleep\n" +
    "[1518-09-29 00:54] falls asleep\n" +
    "[1518-07-05 00:01] Guard #2251 begins shift\n" +
    "[1518-05-24 00:56] wakes up\n" +
    "[1518-03-27 00:02] Guard #277 begins shift\n" +
    "[1518-03-23 00:39] falls asleep\n" +
    "[1518-08-06 00:55] wakes up\n" +
    "[1518-11-20 23:57] Guard #3511 begins shift\n" +
    "[1518-11-21 00:19] wakes up\n" +
    "[1518-03-20 00:03] Guard #277 begins shift\n" +
    "[1518-11-14 00:24] falls asleep\n" +
    "[1518-05-01 23:56] Guard #2591 begins shift\n" +
    "[1518-06-03 00:55] wakes up\n" +
    "[1518-10-03 00:52] wakes up\n" +
    "[1518-05-22 00:10] wakes up\n" +
    "[1518-11-09 23:59] Guard #2083 begins shift\n" +
    "[1518-04-13 00:32] falls asleep\n" +
    "[1518-06-18 23:57] Guard #2251 begins shift\n" +
    "[1518-08-03 00:11] falls asleep\n" +
    "[1518-03-18 00:22] falls asleep\n" +
    "[1518-08-20 00:59] wakes up\n" +
    "[1518-04-26 00:11] falls asleep\n" +
    "[1518-08-31 00:00] Guard #3511 begins shift\n" +
    "[1518-08-07 00:09] falls asleep\n" +
    "[1518-11-05 00:00] Guard #2663 begins shift\n" +
    "[1518-09-11 00:04] Guard #2377 begins shift\n" +
    "[1518-07-14 00:00] Guard #79 begins shift\n" +
    "[1518-10-24 23:56] Guard #79 begins shift\n" +
    "[1518-04-06 23:58] Guard #1259 begins shift\n" +
    "[1518-02-27 00:10] falls asleep\n" +
    "[1518-10-27 23:50] Guard #79 begins shift\n" +
    "[1518-06-27 00:00] falls asleep\n" +
    "[1518-09-24 00:16] falls asleep\n" +
    "[1518-02-17 23:57] Guard #277 begins shift\n" +
    "[1518-07-10 00:36] wakes up\n" +
    "[1518-07-22 00:57] wakes up\n" +
    "[1518-03-19 00:55] falls asleep\n" +
    "[1518-08-28 23:59] Guard #631 begins shift\n" +
    "[1518-06-26 00:25] wakes up\n" +
    "[1518-08-09 00:01] falls asleep\n" +
    "[1518-03-10 00:49] falls asleep\n" +
    "[1518-07-08 00:00] Guard #1259 begins shift\n" +
    "[1518-04-24 00:32] falls asleep\n" +
    "[1518-04-13 00:04] Guard #79 begins shift\n" +
    "[1518-07-04 00:03] Guard #2083 begins shift\n" +
    "[1518-06-16 23:48] Guard #2267 begins shift\n" +
    "[1518-09-12 23:59] Guard #2713 begins shift\n" +
    "[1518-10-21 23:58] Guard #2083 begins shift\n" +
    "[1518-08-15 00:58] wakes up\n" +
    "[1518-09-24 00:48] falls asleep\n" +
    "[1518-07-11 00:46] wakes up\n" +
    "[1518-08-28 00:12] falls asleep\n" +
    "[1518-06-30 00:44] falls asleep\n" +
    "[1518-09-04 23:58] Guard #2377 begins shift\n" +
    "[1518-04-09 00:01] Guard #79 begins shift\n" +
    "[1518-10-02 00:47] falls asleep\n" +
    "[1518-11-19 00:51] falls asleep\n" +
    "[1518-03-21 00:00] Guard #277 begins shift\n" +
    "[1518-04-04 00:46] wakes up\n" +
    "[1518-11-07 23:58] Guard #631 begins shift\n" +
    "[1518-08-03 00:58] wakes up\n" +
    "[1518-11-08 00:30] falls asleep\n" +
    "[1518-03-06 00:11] wakes up\n" +
    "[1518-06-09 23:56] Guard #277 begins shift\n" +
    "[1518-03-26 00:26] falls asleep\n" +
    "[1518-04-08 00:00] Guard #79 begins shift\n" +
    "[1518-03-09 00:02] falls asleep\n" +
    "[1518-09-02 00:25] falls asleep\n" +
    "[1518-03-02 23:56] Guard #1499 begins shift\n" +
    "[1518-08-13 00:26] falls asleep\n" +
    "[1518-09-20 00:02] Guard #563 begins shift\n" +
    "[1518-11-03 00:08] falls asleep\n" +
    "[1518-05-06 00:44] wakes up\n" +
    "[1518-08-08 00:00] Guard #631 begins shift\n" +
    "[1518-06-07 00:27] wakes up\n" +
    "[1518-06-07 00:34] falls asleep\n" +
    "[1518-03-17 00:57] wakes up\n" +
    "[1518-03-10 00:04] falls asleep\n" +
    "[1518-06-12 00:44] wakes up\n" +
    "[1518-07-02 00:57] wakes up\n" +
    "[1518-03-29 00:03] Guard #2251 begins shift\n" +
    "[1518-08-28 00:03] Guard #1499 begins shift\n" +
    "[1518-08-26 00:55] wakes up\n" +
    "[1518-04-15 00:40] wakes up\n" +
    "[1518-04-17 23:56] Guard #2251 begins shift\n" +
    "[1518-07-25 00:52] wakes up\n" +
    "[1518-04-28 00:35] wakes up\n" +
    "[1518-10-16 00:42] wakes up\n" +
    "[1518-04-22 00:43] wakes up\n" +
    "[1518-03-03 00:56] wakes up\n" +
    "[1518-10-18 23:58] Guard #2663 begins shift\n" +
    "[1518-09-02 00:06] falls asleep\n" +
    "[1518-08-22 00:58] wakes up\n" +
    "[1518-03-04 00:00] Guard #2153 begins shift\n" +
    "[1518-03-23 00:00] Guard #2251 begins shift\n" +
    "[1518-02-28 00:04] Guard #2251 begins shift\n" +
    "[1518-02-16 00:50] wakes up\n" +
    "[1518-04-11 00:56] falls asleep\n" +
    "[1518-10-24 00:41] wakes up\n" +
    "[1518-08-08 00:49] wakes up\n" +
    "[1518-03-05 00:24] wakes up\n" +
    "[1518-06-24 23:57] Guard #3541 begins shift\n" +
    "[1518-05-21 00:24] falls asleep\n" +
    "[1518-08-10 00:47] falls asleep\n" +
    "[1518-03-08 00:22] falls asleep\n" +
    "[1518-09-09 00:10] falls asleep\n" +
    "[1518-04-10 23:59] Guard #79 begins shift\n" +
    "[1518-06-22 23:57] Guard #3079 begins shift\n" +
    "[1518-08-23 00:56] wakes up\n" +
    "[1518-09-18 00:40] wakes up\n" +
    "[1518-10-25 00:17] wakes up\n" +
    "[1518-05-30 00:56] falls asleep\n" +
    "[1518-10-30 00:04] Guard #2591 begins shift\n" +
    "[1518-09-22 00:57] wakes up\n" +
    "[1518-08-07 00:20] wakes up\n" +
    "[1518-10-26 00:11] falls asleep\n" +
    "[1518-03-08 00:37] falls asleep\n" +
    "[1518-04-07 00:55] wakes up\n" +
    "[1518-05-05 00:31] wakes up\n" +
    "[1518-03-17 00:53] falls asleep\n" +
    "[1518-05-14 00:42] falls asleep\n" +
    "[1518-06-30 00:21] wakes up\n" +
    "[1518-08-05 00:43] falls asleep\n" +
    "[1518-06-09 00:45] falls asleep\n" +
    "[1518-05-06 00:03] Guard #2153 begins shift\n" +
    "[1518-04-30 00:24] falls asleep\n" +
    "[1518-05-28 00:43] wakes up\n" +
    "[1518-03-07 00:26] wakes up\n" +
    "[1518-06-12 00:03] Guard #2377 begins shift\n" +
    "[1518-06-12 00:19] falls asleep\n" +
    "[1518-03-01 00:55] wakes up\n" +
    "[1518-07-10 00:02] Guard #1709 begins shift\n" +
    "[1518-07-31 00:42] wakes up\n" +
    "[1518-08-05 23:57] Guard #2267 begins shift\n" +
    "[1518-07-31 00:51] falls asleep\n" +
    "[1518-04-11 23:58] Guard #631 begins shift\n" +
    "[1518-08-04 00:48] falls asleep\n" +
    "[1518-11-08 00:58] wakes up\n" +
    "[1518-05-29 00:11] falls asleep\n" +
    "[1518-05-17 00:08] falls asleep\n" +
    "[1518-10-09 00:04] Guard #983 begins shift\n" +
    "[1518-03-09 00:44] falls asleep\n" +
    "[1518-03-19 00:18] falls asleep\n" +
    "[1518-07-10 00:30] falls asleep\n" +
    "[1518-07-01 00:56] wakes up\n" +
    "[1518-09-08 00:05] falls asleep\n" +
    "[1518-05-23 00:58] wakes up\n" +
    "[1518-11-11 00:34] wakes up\n" +
    "[1518-07-25 00:04] Guard #277 begins shift\n" +
    "[1518-09-16 00:27] falls asleep\n" +
    "[1518-05-29 00:01] Guard #2267 begins shift\n" +
    "[1518-08-27 00:47] falls asleep\n" +
    "[1518-10-12 00:03] Guard #2251 begins shift\n" +
    "[1518-04-22 00:29] falls asleep\n" +
    "[1518-07-01 00:00] Guard #2083 begins shift\n" +
    "[1518-09-03 00:45] wakes up\n" +
    "[1518-02-15 00:11] falls asleep\n" +
    "[1518-11-12 00:14] falls asleep\n" +
    "[1518-11-07 00:31] falls asleep\n" +
    "[1518-10-30 23:57] Guard #2689 begins shift\n" +
    "[1518-05-16 00:56] wakes up\n" +
    "[1518-10-17 00:15] falls asleep\n" +
    "[1518-07-16 00:03] falls asleep\n" +
    "[1518-04-30 00:26] wakes up\n" +
    "[1518-02-21 00:37] falls asleep\n" +
    "[1518-03-20 00:39] wakes up\n" +
    "[1518-04-22 00:47] falls asleep\n" +
    "[1518-04-05 00:12] falls asleep\n" +
    "[1518-05-07 00:07] wakes up\n" +
    "[1518-06-16 00:41] falls asleep\n" +
    "[1518-02-21 00:45] wakes up\n" +
    "[1518-10-02 00:44] wakes up\n" +
    "[1518-07-21 00:09] wakes up\n" +
    "[1518-05-25 23:59] Guard #2153 begins shift\n" +
    "[1518-09-13 23:56] Guard #1499 begins shift\n" +
    "[1518-10-06 00:39] falls asleep\n" +
    "[1518-11-12 00:18] wakes up\n" +
    "[1518-05-04 00:00] Guard #2377 begins shift\n" +
    "[1518-11-05 00:23] falls asleep\n" +
    "[1518-06-15 00:41] falls asleep\n" +
    "[1518-06-07 00:23] falls asleep\n" +
    "[1518-09-18 00:01] Guard #277 begins shift\n" +
    "[1518-10-24 00:56] wakes up\n" +
    "[1518-06-16 00:03] Guard #2251 begins shift\n" +
    "[1518-08-09 00:54] wakes up\n" +
    "[1518-11-11 00:28] falls asleep\n" +
    "[1518-08-19 00:00] Guard #983 begins shift\n" +
    "[1518-09-21 00:53] wakes up\n" +
    "[1518-09-23 00:45] wakes up\n" +
    "[1518-11-22 00:15] wakes up\n" +
    "[1518-09-12 00:46] falls asleep\n" +
    "[1518-09-27 00:29] falls asleep\n" +
    "[1518-08-15 00:38] falls asleep\n" +
    "[1518-04-15 00:54] falls asleep\n" +
    "[1518-10-31 00:22] falls asleep\n" +
    "[1518-04-16 00:01] Guard #1109 begins shift\n" +
    "[1518-05-07 00:14] wakes up\n" +
    "[1518-06-05 00:42] wakes up\n" +
    "[1518-06-16 00:45] wakes up\n" +
    "[1518-07-08 00:41] wakes up\n" +
    "[1518-04-21 23:46] Guard #2663 begins shift\n" +
    "[1518-09-02 00:22] wakes up\n" +
    "[1518-03-08 00:00] Guard #631 begins shift\n" +
    "[1518-08-25 00:46] falls asleep\n" +
    "[1518-06-12 00:06] falls asleep\n" +
    "[1518-02-22 00:37] wakes up\n" +
    "[1518-09-25 00:45] wakes up\n" +
    "[1518-04-15 00:01] Guard #2797 begins shift\n" +
    "[1518-02-19 00:04] Guard #2267 begins shift\n" +
    "[1518-07-20 00:46] wakes up\n" +
    "[1518-11-23 00:00] Guard #277 begins shift\n" +
    "[1518-09-24 00:04] Guard #2251 begins shift\n" +
    "[1518-08-08 23:47] Guard #1709 begins shift\n" +
    "[1518-07-13 00:28] wakes up\n" +
    "[1518-04-06 00:37] wakes up\n" +
    "[1518-03-29 00:36] falls asleep\n" +
    "[1518-03-16 23:54] Guard #631 begins shift\n" +
    "[1518-09-28 00:57] wakes up\n" +
    "[1518-11-16 00:49] wakes up\n" +
    "[1518-06-08 00:50] wakes up\n" +
    "[1518-06-17 00:18] wakes up\n" +
    "[1518-06-06 00:35] wakes up\n" +
    "[1518-11-08 00:57] falls asleep\n" +
    "[1518-08-23 23:54] Guard #2663 begins shift\n" +
    "[1518-10-27 00:00] falls asleep\n" +
    "[1518-05-12 00:57] wakes up\n" +
    "[1518-11-17 00:13] falls asleep\n" +
    "[1518-07-27 00:57] falls asleep\n" +
    "[1518-05-03 00:10] falls asleep\n" +
    "[1518-03-08 23:51] Guard #631 begins shift\n" +
    "[1518-11-06 00:11] falls asleep\n" +
    "[1518-02-20 00:46] falls asleep\n" +
    "[1518-06-20 00:37] wakes up\n" +
    "[1518-11-14 00:11] falls asleep\n" +
    "[1518-05-08 23:59] Guard #2663 begins shift\n" +
    "[1518-05-18 00:29] wakes up\n" +
    "[1518-09-05 00:59] wakes up\n" +
    "[1518-10-13 00:03] Guard #2251 begins shift\n" +
    "[1518-11-14 00:58] wakes up\n" +
    "[1518-04-14 00:29] falls asleep\n" +
    "[1518-02-23 00:58] wakes up\n" +
    "[1518-03-18 00:03] Guard #2137 begins shift\n" +
    "[1518-10-23 00:53] wakes up\n" +
    "[1518-05-15 00:46] wakes up\n" +
    "[1518-06-02 00:03] Guard #3511 begins shift\n" +
    "[1518-07-09 00:46] wakes up\n" +
    "[1518-07-24 00:39] wakes up\n" +
    "[1518-07-06 23:52] Guard #2137 begins shift\n" +
    "[1518-02-28 23:46] Guard #3511 begins shift\n" +
    "[1518-08-25 00:10] falls asleep\n" +
    "[1518-06-11 00:01] Guard #2137 begins shift\n" +
    "[1518-06-02 00:13] falls asleep\n" +
    "[1518-08-05 00:50] wakes up\n" +
    "[1518-11-22 00:55] wakes up\n" +
    "[1518-10-09 00:13] falls asleep\n" +
    "[1518-07-08 23:46] Guard #2153 begins shift\n" +
    "[1518-04-10 00:50] wakes up\n" +
    "[1518-04-13 23:58] Guard #631 begins shift\n" +
    "[1518-05-07 00:54] falls asleep\n" +
    "[1518-06-19 00:31] falls asleep\n" +
    "[1518-08-24 00:04] falls asleep\n" +
    "[1518-04-23 00:37] wakes up\n" +
    "[1518-09-18 23:46] Guard #2377 begins shift\n" +
    "[1518-07-21 00:27] falls asleep\n" +
    "[1518-03-28 00:31] falls asleep\n" +
    "[1518-05-18 00:32] falls asleep\n" +
    "[1518-05-04 00:48] wakes up\n" +
    "[1518-07-11 00:57] falls asleep\n" +
    "[1518-04-06 00:55] wakes up\n" +
    "[1518-08-13 00:43] wakes up\n" +
    "[1518-03-24 00:14] falls asleep\n" +
    "[1518-05-25 00:00] Guard #2713 begins shift\n" +
    "[1518-04-19 23:54] Guard #2153 begins shift\n" +
    "[1518-04-13 00:25] falls asleep\n" +
    "[1518-04-19 00:51] falls asleep\n" +
    "[1518-11-23 00:14] falls asleep\n" +
    "[1518-10-08 00:00] falls asleep\n" +
    "[1518-09-21 00:47] falls asleep\n" +
    "[1518-05-02 00:09] falls asleep\n" +
    "[1518-04-07 00:18] falls asleep\n" +
    "[1518-07-30 00:03] falls asleep\n" +
    "[1518-08-21 00:03] Guard #2137 begins shift\n" +
    "[1518-11-13 23:59] Guard #2267 begins shift\n" +
    "[1518-08-14 00:51] wakes up\n" +
    "[1518-08-26 00:47] falls asleep\n" +
    "[1518-04-22 00:05] wakes up\n" +
    "[1518-04-24 23:51] Guard #1709 begins shift\n" +
    "[1518-06-13 00:07] wakes up\n" +
    "[1518-03-22 00:41] wakes up\n" +
    "[1518-10-09 00:49] falls asleep\n" +
    "[1518-05-27 00:05] falls asleep\n" +
    "[1518-09-29 23:48] Guard #79 begins shift\n" +
    "[1518-06-01 00:50] wakes up\n" +
    "[1518-05-16 00:35] wakes up\n" +
    "[1518-07-09 00:02] falls asleep\n" +
    "[1518-06-22 00:46] falls asleep\n" +
    "[1518-08-15 00:39] wakes up\n" +
    "[1518-06-26 23:50] Guard #2377 begins shift\n" +
    "[1518-10-01 00:50] wakes up\n" +
    "[1518-07-18 23:49] Guard #277 begins shift\n" +
    "[1518-05-06 23:50] Guard #631 begins shift\n" +
    "[1518-04-09 00:42] wakes up\n" +
    "[1518-08-25 00:39] wakes up\n" +
    "[1518-02-13 23:47] Guard #2797 begins shift\n" +
    "[1518-05-11 00:47] falls asleep\n" +
    "[1518-06-09 00:04] Guard #2137 begins shift\n" +
    "[1518-06-21 00:36] falls asleep\n" +
    "[1518-02-20 00:52] wakes up\n" +
    "[1518-10-29 00:43] wakes up\n" +
    "[1518-08-25 00:51] wakes up\n" +
    "[1518-11-14 00:50] wakes up\n" +
    "[1518-03-22 00:47] falls asleep\n" +
    "[1518-04-26 00:54] wakes up\n" +
    "[1518-05-27 00:41] wakes up\n" +
    "[1518-08-30 00:21] wakes up\n" +
    "[1518-08-14 00:02] Guard #1499 begins shift"; // Input split by newlines

input = input.split("\n");

const regex = /\[(\d+)-(\d+)-(\d+) (\d+):(\d+)\] (.+)/gm;

const events = [];

input.forEach(function (event) {
    let m;

    while ((m = regex.exec(event)) !== null) {
        if (m.index === regex.lastIndex) {
            regex.lastIndex++;
        }

        events.push([Date.parse(Number(m[1]) + "-" + m[2] + "-" + m[3] + "T" + m[4] + ":" + m[5]), m[6]]);
    }
});

events.sort((a, b) => a[0] - b[0]);

let id = -1;
let startTime;
let asleepTime;
let wakeTime;

const idRegex = /(\d+)/gm;

const guards = {};

events.forEach(function(event) {
    let date = new Date(event[0]);

    if (event[1].indexOf("Guard") > -1) {
        let m;

        while ((m = idRegex.exec(event[1])) !== null) {
            if (m.index === regex.lastIndex) {
                regex.lastIndex++;
            }

            id = m[1];
            startTime = date;
        }

    } else if(event[1].indexOf("asleep") > -1) {
        asleepTime = date;
    } else {
        wakeTime = date;

        if (guards.hasOwnProperty(id)) {
            guards[id] += wakeTime.getMinutes() - asleepTime.getMinutes() - 1;
        } else {
            guards[id] = wakeTime.getMinutes() - asleepTime.getMinutes() - 1;
        }
    }
});

let guardArr = Object.keys(guards).map(function(key) {
    return [Number(key), guards[key]];
});

guardArr.sort(function (a, b) {
    return b[1] - a[1];
});


let guardMostSleep = guardArr[0];